...
sync-chart-crds: ## Sync generated CRDs into the Helm chart's crds/ directory.
	mkdir -p charts/nats-auth-operator/crds
	cp config/crd/bases/*.yaml charts/nats-auth-operator/crds/
...
undeploy: ## Undeploy controller from the K8s cluster specified in ~/.kube/config. Call with ignore-not-found=true to ignore resource not found errors during deletion.
	kubectl delete --ignore-not-found=$(ignore-not-found) -f config/manager/manager.yaml
	kubectl delete --ignore-not-found=$(ignore-not-found) -f config/rbac/leader_election_role_binding.yaml
	kubectl delete --ignore-not-found=$(ignore-not-found) -f config/rbac/leader_election_role.yaml
